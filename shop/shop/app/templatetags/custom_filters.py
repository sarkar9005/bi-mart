...
def final_total(cart_items):
    total_discounted = sum(
        (item['product'].price - (item['product'].price * item['product'].offer_percent / 100)) * item['quantity']
        for item in cart_items
    )
    
    handling_charge = 4  # Handling charge is fixed
    delivery_charge = 16 if total_discounted < 200 else 0  # Conditional delivery charge

    grand_total = total_discounted + handling_charge + delivery_charge
    return int(grand_total)  # Ensure the total is an integer
